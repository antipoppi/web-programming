...
// update highscoreSpan with highscore
function updateHighscoreSpan() {
    highscore = localStorage.getItem("highscore");
    if (highscore !== null) {
        document.getElementById("highscore").innerHTML = " - Record: " + highscore.toString() + "s";
...
function start_game() {
    ...
    if (gameCount > 0) init();
    // if difficulty is checked increase ball speed
    if (document.getElementById("diffCbox").checked == true) ball.harder();
    // disable start button
    var element = document.getElementById("startButton");
    element.disabled = true;
    // disable difficulty-checkbox
    var element2 = document.getElementById("diffCbox");
...
// end game, remove listeners and enable start a new game -button and update highscore span
function endGame() {
...
function addHighScore(endTime) {
    ...
    // add highscore to local storage and update it if better
    addHighScoreToLocalStorage(seconds);
}

// add highscore into local storage and inform if record is made
function addHighScoreToLocalStorage(seconds) {
    highscore = localStorage.getItem("highscore");
    if (highscore !== null) {
        if (seconds < highscore) {
            localStorage.setItem("highscore", seconds);
            ctx.fillText("New record!", 70, 500);
            // update highscoreSpan
            updateHighscoreSpan();
        }
    } else {
